        ...
        for (int i = 0; i < scoreCard.getPlayersCount() + 2; i++){ //Plus 2 for Par And Hole # rows.
            ...
            for (int j = 0; j < scoreCard.getCourse().getHoleCount() + 2; j++) { // Plus 2 for Name and Total Columns
                ...
                if (i == 1 && j == 0){ // First Cell in second row is "Par"
                    ...
                    TextView tv = setupTextViewInTable(String.valueOf(scoreCard.getCourse().getParTotal()),applyLayoutWidth(LAYOUT_WIDTH, context), R.drawable.cell_shape_light_green, context);
                    row.addView(tv);
                }

                else if (i == 1 && j != scoreCard.getCourse().getHoleCount()+1){
                    TextView tv = setupTextViewInTable(String.valueOf(scoreCard.getCourse().getParArray()[j-1]),applyLayoutWidth(LAYOUT_WIDTH, context), R.drawable.cell_shape_light_green, context);
                    row.addView(tv);
                }
